...
.PHONY: db
db:
	docker exec -it daiquiri_db psql postgresql://admin:admin@localhost:5432/app

# Connect to the test database (you must run tests first before running this)
.PHONY: db-test
db-test:
	docker exec -it daiquiri_db psql postgresql://admin:admin@localhost:5432/app_test

# Connect to the primary cache
.PHONY: cache
cache:
	docker exec -it daiquiri_cache redis-cli

# Clear the primary cache
.PHONY: cache-clear
cache-clear:
	docker exec -it daiquiri_cache redis-cli flushall

 # Connect to the test cache
.PHONY: cache-test
cache-test:
	docker exec -it daiquiri_cache redis-cli -n 1
